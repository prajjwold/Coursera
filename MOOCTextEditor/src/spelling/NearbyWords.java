...
public class NearbyWords implements SpellingSuggest {
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(!queue.isEmpty()){
			...
			for (String wd : distanceOne(curr, true)) {
	...
	public static void main(String[] args) {
		 ...
		 * NearbyWords any Dictionary implementation you prefer Dictionary d =
		 * new DictionaryHashSet(); DictionaryLoader.loadDictionary(d,
		 * "data/dict.txt"); NearbyWords w = new NearbyWords(d); List<String> l
		 * = w.distanceOne(word, true); System.out.println(
		 * "One away word Strings for for \""+word+"\" are:");
		 * System.out.println(l+"\n");
		 * 
		 * word = "tailo"; List<String> suggest = w.suggestions(word, 10);
