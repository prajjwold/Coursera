...
public class MapGraph {
	...
	public List<GeographicPoint> dijkstra(GeographicPoint start, GeographicPoint goal,
		...
		// setup to begin Dijkstra Algorithm
		HashMap<MapNode, MapNode> parentMap = new HashMap<MapNode, MapNode>();
		PriorityQueue<MapNode> toExplore = new PriorityQueue<>();
		HashSet<MapNode> visited = new HashSet<MapNode>();
		// initialize distance for each node to infinity
		for (MapNode mapNode : pointNodeMap.values()) {
			mapNode.setActualDistance(Double.POSITIVE_INFINITY);
			mapNode.setDistance(Double.POSITIVE_INFINITY);
		}
		// enqueue start node in priority queue with distance 0
			...
			if (!visited.contains(next)) {
				...
				for (MapEdge edge : next.getEdges()) {
					...
					if (!visited.contains(neighbor)) {
						...
						// enqueue {n,distance} into PriorityQueue
						double dist = next.getDistance() + edge.getLength();
				...
				for (MapEdge edge : next.getEdges()) {
					...
					if (!visited.contains(neighbor)) {
						...
						if (dist + currToGoalDist < neighbor.getDistance()) {
							neighbor.setActualDistance(dist);
							neighbor.setDistance(dist + currToGoalDist);
							parentMap.put(neighbor, next);
							toExplore.add(neighbor);
						}
					}
	...
	public static void main(String[] args) {
		...
		//
		// List<GeographicPoint> route = theMap.bfs(new GeographicPoint(1.0,
		// 1.0), new GeographicPoint(8.0, -1.0));
		//
